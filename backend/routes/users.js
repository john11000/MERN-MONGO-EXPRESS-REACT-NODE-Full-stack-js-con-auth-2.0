  ...
  if (token) {
    jwt.verify(token, privateKey, (err, decoded) => {
      if (err) {
        ...
        console.log(decoded)
        Usuario.find({ _id: decoded.id_u._id ? decoded.id_u._id : decoded.id_u })
          .then((data) => {
            console.log(data);
            if(data[0]){
              return res.json({
              ...
              return res.json({respuesta :  "No econtramos informacion"})
            }
...
router.post("/google", function (req, res, next) {
  ...
  Usuario.find({ email: email }).then((r) => {
    console.log(r.length);
    if (r.length !== 0) {
      if (documento == r[0].documento && r[0].estado == "autorizado") {
        ...
        return res.json({ respuesta: token , estado : 1 });
      } else {
        return res.json({
